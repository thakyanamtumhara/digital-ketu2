...
export async function assessReplyPair(anthropic, buyerText, ketuReply) {
  const b = String(buyerText || '').trim().slice(0, 600)
  const k = String(ketuReply || '').trim().slice(0, 600)
  if (!b || !k) return { verdict: 'rejected', reason: 'missing_text' }
  try {
    ...
    if (resp.stop_reason && resp.stop_reason !== 'end_turn') return { verdict: 'pending', reason: 'incomplete_validator_response' }
    if (t === 'YES') return { verdict: 'accepted', reason: 'same_topic' }
    if (/^NO[.!]?$/.test(t)) return { verdict: 'rejected', reason: 'different_topic' }
    return { verdict: 'pending', reason: 'ambiguous_validator_response' }
  } catch (err) {
    console.error('[CorrectionGuard] pairing validation unavailable')
    return { verdict: 'pending', reason: 'validator_unavailable' }
  }
}

export async function replyAnswersBuyer(anthropic, buyerText, ketuReply) {
  return (await assessReplyPair(anthropic, buyerText, ketuReply)).verdict === 'accepted'
}
