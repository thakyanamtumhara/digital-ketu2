...
export async function processIncomingMessage({ whatsappNumber, messages, db, anthropic, settings }) {
  ...
  if (!hasMediaOnly && (AUTO_REPLY_RE.test(mergedText || '') || flaggedAutoReply || isStoreMenuGreeting(mergedText, messages)) && !REAL_ASK_RE.test(mergedText || '')) {
    await createLog(db, conversation.id, mergedText, messageIds, {
...
async function runAiFlow({ whatsappNumber, mergedText, quotedText, conversationId, normalizedText, db, anthropic, settings, startTime, messageIds, imageUrl = null }) {
  ...
  let forcedReply = !!imageBlock || FORCE_REPLY_RE.test(mergedText || '') || isAppStoreLookupProblem(mergedText) || isRecipientReceiptQuestion(mergedText)
