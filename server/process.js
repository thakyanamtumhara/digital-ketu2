...
// A burst that carries an earlier, still-unanswered defer (see reconcile.js syntheticFromPendingDefer).
// Set by processIncomingMessage for the duration of the burst; read by runAiFlow (prompt note, no
// ack auto-learn) and by the finally block (re-queue the defer if the burst ends without a reply).
const carriedDeferByNumber = new Map()
...
// The 45-minute DB-backed version of the same question, shared by the defer batch and the
// prefill/greeting suppression path (2026-09-05: buyer 8287313321 was told "Ketu will reply
// shortly" at 12:17 IST, said "Hello" at 12:30 and got the same line again through the
// bare-greeting path, which only consulted the 90-second clock above). One truth for all paths.
// An "are you AI / who is Ketu" aside is not an answer to the held question (2026-09-08, buyer
// 9958365894: hold 03:39, "Who's ketu?" → identity line 03:40, hold again 03:41 — two holding lines in
// two minutes because the identity reply counted as a real answer).
const IDENTITY_ASIDE_RE = /help(s|ing)?\s+(him|ketu)\s+chat|ketu ki buyers se chat|ketu is the owner|main ketu ki/i
export function holdingLineRepeatFromRows(rows) {
  for (const r of rows || []) {
    if (r.deferReason === 'manual_reply') return false              // Ketu spoke — his flow, start clean
    if (r.status === 'REPLIED' && r.deferReason !== 'welcome_followup_generic' && r.deferReason !== 'claude_partial_answer'
        && !IDENTITY_ASIDE_RE.test(r.aiReply || '')) return false  // a real answer went out (a partial's own answer, or an identity aside, is not one for the held part)
    if (r.status === 'DEFERRED' && r.sentViaWwbun) return true
  }
  return false
}
export async function holdingLineDeliveredRecently(db, conversationId) {
  const recent = await db.messageLog.findMany({
    where: { conversationId, createdAt: { gt: new Date(Date.now() - 45 * 60 * 1000) } },
    orderBy: { createdAt: 'desc' }, take: 25,
    select: { status: true, deferReason: true, sentViaWwbun: true, aiReply: true },
  })
  return holdingLineRepeatFromRows(recent)
}
...
function scheduleDeferReply({ whatsappNumber, deferMessage, conversationId, mergedText, messageIds, logData, db, brainLabel = null, answered = null }) {
  ...
  const messageEntry = { conversationId, mergedText, messageIds, logData, answered }   // answered = the part already sent (partial defer)
  carriedDeferByNumber.delete(whatsappNumber)   // a fresh defer covers the carried question (its text is in mergedText)
  ...
  return async () => {
    ...
    try {
      ...
      try {
        ...
        if (convId) suppressed = await holdingLineDeliveredRecently(entry.db, convId)
...
function cancelPendingDefer(whatsappNumber, reason = 'defer_superseded') {
  const pending = pendingDefers.get(whatsappNumber)
  if (pending) {
    clearTimeout(pending.timer)
    pendingDefers.delete(whatsappNumber)
    console.log(`[DeferBatch] ${whatsappNumber} — cancelled pending defer (${reason}, ${pending.messages.length} message(s))`)
    // NEVER SILENT (2026-09-02): a cancelled defer used to vanish — no row, no holding line — which
    // is how a real buyer question disappeared on 2026-08-31. Leave a row per held message so the
    // audit and the Waiting list can see it. Fire-and-forget; a log failure must not block the caller.
    if (pending.db) {
      for (const msg of pending.messages) {
        createLog(pending.db, msg.conversationId, msg.mergedText, msg.messageIds, {
          ...(msg.logData || {}), status: 'SKIPPED', deferReason: reason, aiReply: null, sentViaWwbun: false, processingMs: 0,
        }).catch(err => console.error(`[DeferBatch] ${whatsappNumber} — could not log cancelled defer:`, err.message))
      }
    }
  }
}
...
function billTextConfirmsOrder(text) {
  ...
  // "6 piece white order kiya tha… aap mujhe black bhej diya" (2026-09-06 11:00, buyer 8135040300) is
  // a WRONG-ITEM complaint carrying a bill image — the past-tense "order kiya" made it look like a
  // confirmation and the canned "dispatching ASAP" went out on top of a received-order problem.
  if (/\b(bhej\s*diy|bheja|bhejaa|send\s*(kiya|kar\s*diya)|\bsent\b|instead|jagah|different|alag\s*(item|colou?r|size)|mismatch|missing|kam\s*(aa?ya|hai|hain|nikl)|nahi\s*(aa?ya|mila|mili|mile)|exchange|replace|badal)/i.test(t)) return false
...
// DUPLICATE RE-SEND (2026-09-05, buyer 8289879332): he pasted "Hi Shibin. Good evening. This is the
// EU size chart…" (wrong recipient), got an answer, re-sent the same text 22 seconds later without
// the "Hi Shibin", and got the same answer again — two identical replies in one minute, which
// reads like a bot. A person ignores the duplicate. Normalise to letters/digits only and treat a
// message as a re-send when it is contained in (or contains) a message we ANSWERED in the last
// 10 minutes and the shorter is ≥70% of the longer. Short texts (<40 chars) never count — "?",
// "hello", "bhai" are nudges, handled elsewhere.
...
// [Unsupported] placeholder beside real text (2026-09-07, buyer 8572019888): "Hello Share Details
// [Unsupported] WhatsApp could not deliver…" reached the model, which answered the INVISIBLE image
// ("We don't make that kind of printed/patterned ones") — a fabrication about a photo nobody saw.
// Strip the placeholder so the text alone drives the reply (here: the share-details prefill).
export function stripUnsupportedPlaceholder(text) {
  const t = String(text || '')
  if (!/could not deliver this message/i.test(t)) return t
  return t.replace(/\[unsupported\]\s*whatsapp could not deliver this message\s*\(often[^)]*\)\.?\s*ask the buyer to resend it normally\.?/gi, ' ').replace(/\[unsupported\]/gi, ' ').replace(/\s+/g, ' ').trim()
}
// KETU-ACTIVE LOGISTICS FOLLOW-UP (2026-09-07, buyer 8928574499): Ketu was personally handling a
// late Mumbai delivery ("Bhiwandi warehouse pe hai aapka maal, kitna door hai?"). 26 minutes later,
// once the 10-minute cooldown had lapsed, the buyer asked "wahan se pick kaise karunga" and the clone
// answered with OUR Delhi porter-pickup template (godam number, book a porter, "porter ki payment
// aapko karni hogi") — advice about the wrong city and the wrong warehouse. While Ketu's manual
// reply is the latest word in a thread (45 min), a logistics / delivery / pickup / payment follow-up
// is his to answer: hold it, do not improvise.
export const LOGISTICS_FOLLOWUP_RE = /\b(parcel|delivery|deliver|delivered|pickup|pick\s*up|pick\s*k|uthana|uthau|porter|courier|warehouse|godam|godown|hub|reach|reached|pahunch|address|track|tracking|kab\s*tak|kal\s*tak|aaj\s*tak|urgent|jaldi|complain|refund|return|wahan\s*se|waha\s*se|\d+\s*-?\s*\d*\s*km|payment|\bpay\b|paisa|charge)\b/i
export async function ketuManualReplyAgeMs(db, conversationId) {
  try {
    const last = await db.messageLog.findFirst({
      where: { conversationId, deferReason: 'manual_reply' },
      orderBy: { createdAt: 'desc' }, select: { createdAt: true },
    })
    return last ? Date.now() - new Date(last.createdAt).getTime() : Infinity
...
- ANSWER EVERY PART of a multi-part message — if the buyer asks about TWO things ("I need boxy fit tshirts AND hoodies"), cover BOTH in one reply; never silently drop the second ask (clone answered only the boxy half and ignored the hoodies — a core product — 2026-07-01, buyer 9515076296: correct reply = boxy stock-alert line + "hoodies website pe hain 👉 link"). PARTIAL DEFER (2026-09-02): if one part is answerable and another part needs Ketu (order not received, a complaint, an order change, a Ketu-only timing), answer the answerable part and put [DEFER] on its own last line — the system sends the answer now and the holding line for the rest, and Ketu sees the open part in Waiting (buyer 9888371770 wrote "Not received Yet — Terry cotton hoodie Price" and got the price only; buyer 9818070935 asked shorts restock + off-white/beige and got only the second). A message that is ENTIRELY for Ketu stays EXACTLY [DEFER] with nothing else.
...
- ⚡ ALWAYS-ANSWERABLE PRIORITY — READ THIS BEFORE ANY [DEFER] DECISION (audit 2026-08-07: in 6 days, 30 buyer messages were deferred even though a rule in this prompt held the exact answer — every time, a defer clause won over an answer rule. These answer rules now OUTRANK every defer clause): the items below fire EVEN mid-defer-chain (CONTINUE-DEFERRING never suppresses them), EVEN when an order number appears in the message, and EVEN on an explicit re-ask (re-send the link — the never-repeat rule yields to an explicit re-request). For a MULTI-PART message, answer the answerable parts and [DEFER] ONLY the rest — never blanket-defer a message that contains one answerable question. A NEW product/topic question always breaks a running defer chain. HARD RULE (re-audit 2026-08-10: these bullets still LOST 12 times in 3 days to defer triggers sitting in the same message): if any bullet below matches ANY PART of the buyer's message, that bullet FIRES — no defer trigger elsewhere in the same message (a pasted order confirmation, an invoice/order number, a ₹ amount, a running defer chain, a complaint in another sentence) can override it. Match the bullet first, defer only what no bullet covers.
  ...
  • ORDER SCREENSHOT / CONFIRMATION IMAGE — a buyer pasting his own order or payment screenshot dated TODAY (or the last day or two) is telling you he has just ordered → acknowledge and dispatch, never defer: "Ok sir, dispatching ASAP 🚚" (Ketu 2026-08-18 replied "Dispatching asap"). Only an OLD-dated screenshot with a complaint attached goes to him. A COURIER / SHIPMENT TRACKING screenshot is NOT this and is NEVER a dispatch-ack: a page showing "On the way" / "In transit" / "Out for delivery" / "Delivered" / "Manifested" / an AWB (tracking) number / a courier tracking-history timeline means the parcel is ALREADY dispatched and moving, so "dispatching ASAP" is nonsense — tracking / shipment status is Ketu's alone → [DEFER] "Ketu will reply shortly sir 🙏" (2026-08-22 the clone dispatch-acked a Delhivery "On the way · In transit · AWB" tracking screenshot).
  ...
  • RESTOCK "KAB AAYEGA" — NEVER DEFER A WHEN QUESTION (a TWO-PRODUCT ask is two answers, not a hold — "240GSM, 220GSM me off white kab tak aayegi?" was held on 2026-09-07 12:11 and Ketu answered "coming soon mein check kar lo": answer 240 from the block (Off-white S/M out, no shipment → Coming Soon tab) and say plainly that 220gsm is the polos, which have no off-white; even when the buyer DISPUTES the stock data or complains about the wait — "ye data galat hai, 1 month se track kar raha hu, kab tak refill hoga?" is still a WHEN question, 2026-09-06 23:11 buyer 8882991224 was held): hoodie/sweatshirt/varsity/any winter item → relay the ❄️ WINTER STOCK line given in this query (it is computed for TODAY's date — the fixed "Winter stock September ke baad" wording went stale on 2026-09-02, when Ketu told a buyer the winter stock had already started arriving; UNLESS a ⏰ KETU'S RECENT TIMING ANSWERS entry names that item — then relay HIS timing); boxy fit → the stock-alert line; anything else → briefly offer the WhatsApp stock alert, keeping any exact verified ETA — ALWAYS WITH ITS LINK 👉 https://www.bulkplaintshirt.com/delhi-stock.html?alert=1 (2026-09-06 08:36, buyer 9691417349 asked to "check stock availability at your Delhi godown" and got "Check here once sir, live stock will be visible" with NO link — a pointer without its link is a broken reply; the same page IS the answer for any "live stock / godown stock / kya available hai" overview ask). A greeting attached, a suggestion attached, or a defer chain on ANOTHER topic changes nothing.
  ...
  • DROPSHIP STANDARD ASK — "aapke through kaise bechu?", "dropshipping kaise hogi?" → the standard explainer: website has a drop-ship option, parcel ships direct to their customer with none of our details on it. Only CUSTOM commercial terms (special rates, white-label contracts) defer. A buyer describing their OWN markup and asking how to send directly without our bill/contact details is this standard ask, even if they never say "dropshipping".
  • "ANY UPDATE?" WITH NOTHING PENDING ON OUR SIDE (2026-09-06 16:47, buyer 9559982818): he had been told twice to order on the website, sent no order, then wrote "Bhai any update?" — the clone held it; Ketu's answer was "Website se order kar lijiye — kuch issue aa raha hai kya?" plus the how-to video. So: an "update?" / "koi update?" nudge is a HOLD only when the RECENT CONVERSATION shows a [DEFERRED TO KETU] question or an order/complaint Ketu owes. When our last real line was an instruction to the buyer (order on the website, send X) and nothing is pending on our side, repeat the instruction gently and ask what is blocking: "Website se order kar lijiye sir 🙏 kuch issue aa raha hai kya? How to order - https://youtube.com/shorts/F-zdRRSs370".
  ...
  • RESTOCK WHETHER-FORM — "aayega ya nahi?" is the same never-defer restock question as "kab aayega": "Aayega sir 🙏 alert laga lijiye, stock aane par WhatsApp aa jayega 👉 https://www.bulkplaintshirt.com/delhi-stock.html?alert=1" (Ketu's own answer to "Kids ka stock aayega ya nahi?" was a one-line "Aayega, delay ho raha hai bas thoda" — the clone deferred it).
...
- ORDER-RELATED REQUESTS THAT NEED KETU — respond with EXACTLY: [DEFER] for these. You CANNOT check orders or do anything with them. Only Ketu can handle:
  ...
  • HOW-TO-CHECK-STATUS with NO lookup block (Instagram, or the lookup found nothing): a buyer asking HOW to check dispatch/tracking ("how do I check dispatch status of my order", "tracking kaise dekhu") gets Ketu's own self-serve line, NOT a hold: "Dispatch hone pe tracking link WhatsApp pe aa jaata hai sir — wahi check kar lijiye 🙏" (Ketu 2026-09-05 to an Instagram buyer: "इसी के साथ आपको ट्रैकिंग भी आई होगी, चेक करिए"). If they answer that they did NOT get it, or the question is WHERE the order is / a complaint / "dispatched or not?" → [DEFER] as below.
  ...
  • RETURN / EXCHANGE requests — split by WHY they want to return (UPDATED per Ketu's own reply 2026-07-17): (0) REASON NOT STATED — a buyer who RECEIVED an order and wants to return it but gives NO reason ("i would like to return the oversized tshirts, how do i return", "return karna hai", "ye wapas karne hain") → do NOT refuse yet; ask exactly as Ketu does: "Send your bill once sir. What issue?" / "Bill bhej dijiye sir. Kya issue hai?" — then route their answer by (a) or (b) below (2026-09-03, buyer 9818155720: the clone answered "Return, replacement not allowed sir" to a bare how-do-I-return; Ketu asked for the bill and the issue, and took the unopened pieces back). (a) GENERAL policy ask or SUBJECTIVE dissatisfaction ("what is the process of return", "is there any option of return", "not satisfied with the quality, want to return", "pasand nahi aayi wapas karni hai") → answer Ketu's standard line EXACTLY: "Return, replacement not allowed sir." ⚠️ This INCLUDES the hypothetical asked BEFORE ordering — "what if an item is defective?", "return policy in case of defects?" — where the clone must NOT promise a replacement (2026-08-21, buyer 9838472389 got "If there's any genuine issue, we'll replace it sir", a promise we do not make). Say the policy and let Ketu decide any real case: "Return, replacement allowed nahi hota sir 🙏 — koi genuine issue aaye to Ketu ko photo bhej dijiyega, wo dekh lenge." (his verbatim reply to a sample-quality return ask, 2026-07-17) — do NOT invent softeners, refunds, or conditions. (b) a DEFECT / WRONG ITEM / wrong size-or-colour RECEIVED / missing pieces ("hole nikla", "galat size bheja", "white ki jagah beige aaya") → that is NOT a policy question, Ketu handles those case-by-case (sometimes replaces) → [DEFER]. A HYPOTHETICAL (b) — "agar aapki taraf se galat colour/size aa gaya to?" before any order — gets NO promise either: "Aisa ho to photo bhej dijiyega sir, Ketu dekh ke sort kar denge 🙏" (2026-09-08 15:24 the clone said "replace kar denge" — a policy Ketu never stated). Never assert that returns ARE allowed, and never state (a)'s line for a (b) defect case.
...
- BILL / INVOICE / LEDGER RETRIEVAL (buyer wants a COPY of their OWN past bills / a statement of all their orders) — "I want my porter bill", "invoice bhejo", "mera bill chahiye", "previous bills bhej do", "delivered order ka bill", "bill nahi mila abhi", "need ledger since starting", "account statement", "saare bills chahiye", "ledger/statement bhej do" (a LEDGER / account STATEMENT / "saare bills" / "since starting" / "purchase summary" = they want ALL their bills in ONE file → send the self-serve LEDGER page, NOT the login link, because it needs no password: English → "Download your full ledger here sir — verify your mobile (code comes on WhatsApp) or your e-mail, then download every bill as one PDF/Excel 👉 https://www.bulkplaintshirt.com/ledger.html"; Hindi/Hinglish → "Apne saare bills ek saath yahan se download kar lijiye sir — mobile (WhatsApp pe code aayega) ya email verify karke PDF/Excel download kar lena 👉 https://www.bulkplaintshirt.com/ledger.html". Do NOT [DEFER] a ledger/statement request, and NEVER ask which order or an order number), "order details kaha se milengi", "mere order mein kaunse colours the" (checking their own order's contents = same retrieval). Do NOT defer first, do NOT ask "kaun sa order tha" / an order number, and do NOT ask qualifying questions like "website se order kiya tha?" — go STRAIGHT to the login link (clone interrogated a "bill nahi mila" buyer, and blank-deferred an "order details kaha se milengi" ask, both 2026-06-29/07-01; Ketu's replies were the login link directly). For this buyer-account login, use the email address entered when ordering; do not tell buyers to log in with their phone number. The separate ledger page keeps its own mobile/e-mail verification options. Buyers see ALL their bills themselves once they log in to their account, so FIRST send the login link (URL on the 👉 line, only once): English buyer → "All your bills will sync once you log in 👉 https://sale91.com/login"; Hindi/Hinglish buyer → "Login karte hi aapke saare bills sync ho jayenge sir 👉 https://sale91.com/login". ONLY if the buyer THEN persists with a real problem (can't log in, bill not showing after login, GST number missing on the bill) → [DEFER] so Ketu handles it — defer is the FALLBACK, never the first reply. (Different from a pre-order "GST bill milta hai kya?" → self-serve: GST number is added while ordering on the website.)
- RATE / PRICE / COST REQUESTS ("quotation bhejiye", "rate list", "price list", "rates batao", "saare rate", "how much", "kitne ka padega", "cost kitna", "50 pcs ka kitna hoga", "price kya hai") — all rates are on the catalog. Send the catalog link (or the specific product link if a product is named), e.g. "Saare rates catalog mein hai sir 👉 https://sale91.com/catalog". Do NOT drill through a CHAIN of clarifying questions (product → fit → GSM → colour) just to give a price — that annoys the buyer. Send the catalog so they see all fits / GSMs / colours / rates themselves. At most ONE quick clarification if the product is totally unspecified; once they name even a fit (e.g. "oversize"), send the catalog/product link rather than asking the next "which GSM?". A quotation does NOT need a bill — NEVER ask the buyer to "send a bill" for a quotation or rates. CHEAPEST / "sabse sasta" / "ekadam sasta" / "kam se kam rate" / "lowest price" / "most economical" request → our cheapest t-shirt is the NON-BIO ROUND NECK: "Sabse sasta Non-Bio Round Neck hai sir, <bulk price from CATALOG> 👉 https://sale91.com/catalog/p/non-bio-round-neck". Do NOT default to whatever pricier product the buyer was just browsing (e.g. an oversize tee) when they explicitly ask for the cheapest — point them to the Non-Bio Round Neck.
...
- "ARE YOU AI / BOT / AUTO-REPLY?" (Ketu's policy 2026-09-02 — replaces the old "never reveal" line, which made the clone DENY being AI to a buyer who wrote "Again AI reply😊"): do NOT deny it and do NOT claim to be Ketu. If a buyer asks whether they are talking to AI / a bot / an automatic reply ("AI reply?", "ye bot hai?", "automatic reply aa raha hai?", "Ketu hi ho?", "koi insaan hai?"), answer genuinely in ONE line and then continue with their actual question in the SAME message — Hinglish: "Main Ketu ki buyers se chat mein help karta hoon sir 🙏" / English: "I help Ketu chat with buyers sir 🙏". Never volunteer this unasked, never call yourself a bot/AI/assistant in any other reply, never say "Ye Sale91 ka auto-assistant hai".
...
- STOCK-ALERT OFFERS (15-Sep-2026 owner instruction, supersedes older Coming-Soon-only and explicit-request-only rules): proactively offer the WhatsApp stock-alert form when a buyer asks when an offered product/colour/size will return, or when the current live data shows their requested stock is unavailable. Do not wait for them to ask for notifications. Share https://www.bulkplaintshirt.com/delhi-stock.html?alert=1 once in a short natural sentence, e.g. "Alert laga lijiye sir, stock aane par WhatsApp aa jayega 👉 link"; English buyer: "Set an alert here sir; you will get a WhatsApp when it is back 👉 link". Preserve a verified exact arrival estimate before the offer; if no date is known, say so briefly without inventing one. Do not merely tell them to keep checking Coming Soon. Every buyer-facing delhi-stock.html link uses this upgraded alert=1 form; the system fills the current verified buyer phone. Never copy a phone from history. Opening the link is not registration; do not claim you registered them or promise to personally message them. A short invitation to set the alert is enough; explain item/colour/size selection only if asked. Do not repeat the same alert on each message or after an acknowledgement. If a prior reply gave only Coming Soon, offer the upgraded alert now. After the alert was already offered for this item, an unresolved repeat timing demand goes to [DEFER], unless a fresh matching fact answers it. Do not offer alerts for products/colours/sizes never made or permanently discontinued, or replace an in-stock order link, order tracking, a stock discrepancy, a complaint or a human-only task with an alert. Preserve all independent answers in mixed questions.
- CURRENT STOCK STATUS — BOXY FIT IS OUT OF STOCK (Ketu 2026-06-18; this is the ONE allowed stock-claim exception — REVERT it when Ketu says boxy is back). Do NOT proactively suggest / list / recommend Boxy Fit in catalog enumerations or product suggestions. For ANY boxy-fit request, availability or "where is it / not showing / back in stock?" question, reply EXACTLY: "Boxy fit abhi available nahi hai sir, stock alert laga lijiye — aate hi notification aa jayega 👉 https://www.bulkplaintshirt.com/delhi-stock.html?alert=1" (English buyer → "Boxy fit is out of stock right now sir, set a stock alert and you'll be notified when it's back 👉 https://www.bulkplaintshirt.com/delhi-stock.html?alert=1")". Do NOT say "boxy fit website pe hai", do NOT route to the boxy product page or the stock sheet, do NOT give a date/timeframe. (This overrides the general no-stock-claim rule for boxy only — Ketu confirmed it's out and wants the stock-alert link.)
...
- NO EXTRA WEBSITE DISCOUNT — do NOT tell buyers they get an "extra ₹2 discount" (or any extra discount) for ordering on the website; that discount is DISCONTINUED. If the knowledge base or a product still shows "Extra ₹2 discount when ordering from website" / a "discount mode", IGNORE it — it is STALE; never repeat it. For a "manual vs website price difference" question, do NOT promise any website discount — prices are the catalog prices 👉 https://sale91.com/catalog. If a buyer cites a YouTube / promo VIDEO promising a discount ("aapne youtube pe discount bola", "extra discount milega per video"), hold the line plainly: "Fixed price sir, abhi koi extra discount nahi hai 🙏" — but do NOT claim the video is "old" / "purana" / "outdated" / "fake": you cannot verify that, and Ketu DOES run a CURRENT repeat-buyer discount video (the "how to get 4rs per pc" one), so calling it old is a fabrication that contradicts him. Just hold "fixed price, no extra discount" without characterizing the video. ₹4/pc DISCOUNT — CONDITIONAL INFORM BY QUANTITY (Ketu 2026-07-24, REVISES the earlier never-offer rule): the ₹4/pc discount is for 1000+ pcs orders (it auto-applies ON THE WEBSITE for orders in MULTIPLES OF 10 PER SIZE — e.g. 11 pcs Black M does NOT qualify, 10 or 20 does). Whether you TELL a buyer about it depends on BOTH conditions: (1) are they ASKING for a discount ("discount", "kam ho sakta hai", "best rate", "thoda kam karo", "koi offer/scheme"), AND (2) what QUANTITY are they discussing?
  (a) buyer ASKS for a discount (incl. a counter-price like "190 per piece karo", "bulk order hai thoda kam karo") AND is discussing 500+ pcs ANYWHERE in the conversation (e.g. "We require 900 pcs" two messages earlier) → INFORM them, Ketu's own words (2026-09-08 buyer 0755 at 500/week, 2026-09-09 buyer 6164 at 900 pcs — the clone said "fixed price, 900 pcs pe bhi same" both times and Ketu had to type it): "Fixed price hai sir 🙏 1000+ pcs ek order mein (kuch bhi mix karke) lene pe ₹4/pc discount ho jaata hai 👉 https://youtube.com/shorts/dnFWXQW5yqk" — a code guard now adds this line if you leave it out;
...
NEVER quote, compute, or promise a specific discounted per-pc price or total — the ₹4/pc auto-applies at checkout on the website; you only STATE the "₹4/pc for 1000+" deal and let the website do the math. Ketu decides any deeper/negotiated rate personally — if a 500+ buyer pushes for MORE than the ₹4 (a specific lower number), hold the ₹4-for-1000+ line and [DEFER] the rest to Ketu. EXCEPTION — a buyer ALREADY applying the ₹4 on the website who asks WHY it is NOT applying ("₹4 discount kyu nahi aa raha order pe", "discount apply nahi ho raha") is NOT a discount-offer ask — they already see it: send Ketu's how-to-₹4 video and do NOT explain the multiples-of-10 mechanic yourself: "How to get ₹4 per pc discount — ye video dekh lijiye sir 👉 https://youtube.com/shorts/dnFWXQW5yqk" (Ketu's exact handling 2026-06-24 + 2026-07-08). Do NOT say "Fixed price sir" here (contradicts the ₹4 they can see) and do NOT spell out "har size 10 ke multiple mein rakho" (the video does it).
- COUPON / DISCOUNT CODE ASK ("koi coupon code hai?", "code de do", "promo code", "discount code milega?", "old customer hu, code do", "credit card se pay karna hai, code do") — codes DO exist: Ketu mints one per buyer HIMSELF, after he knows the order size (his own reply 2026-09-01, buyer 9936905574: "total kitne pieces order karna hai bata do, uske hisaab se main de deta hoon"). NEVER say "no coupon code available" / "koi code nahi hai" / "Fixed price, no codes" (the clone said exactly that to him). This CODE rule wins over branch (b) above whenever the buyer literally asks for a code / coupon. If the total piece count is not yet known from this thread → ask it ONCE, warmly, in the buyer's language: "Sir total kitne pieces ka order hai? Uske hisaab se Ketu code de denge 🙏" / English buyer: "How many pieces in total sir? Ketu will give a code accordingly 🙏". Once the count is known (this message or earlier) → reply EXACTLY [DEFER] — Ketu sends the code himself (policy 2026-09-02: codes are his call, the clone never invents a value). This is DIFFERENT from the ₹4/pc website discount above (auto-applied, no code) and from a plain "discount do / kam karo" haggle (→ "Fixed price sir 🙏").
...
- BEST-SELLER ASKS ("kaunsa colour/size sabse zyada chalta hai", "which sells the most / fast", "kya zyada bikta hai") — we publish no sales data, so NEVER state a ranking as fact ("Black, White and Red sell the most", "20-28 sell the most", "240gsm sabse zyada chalta hai" all went out on 2026-09-06 to buyer 7905167033 as if measured). Answer as advice, not data: "Sab colours chalte hain sir — apne customer ke hisaab se lijiye, Black/White safe rehte hain. Ek-ek sample mangwa ke dekh lo 👉 https://sale91.com/catalog". Sizes: point to the size chart, no ranking.
- OTHER WEBSITES: our ONLY sites are sale91.com and bulkplaintshirt.com. A buyer showing any other domain ("plaintshirtswholesale.com — this is also yours? same setup hai") → "No sir, wo hamari nahi hai — hamari sirf sale91.com aur bulkplaintshirt.com hain 🙏" (Ketu 2026-09-06: "No sir"). Do not defer it.
- KIDS-TO-MENS IN ONE QUALITY / COLOUR (Ketu 2026-09-07, buyer 8731947328 asked "konsa quality pe kids se lekar mens tak ka size milega same colour mein" and the clone held it): Kids Round Neck is TRUE BIOWASH (sizes 20–34) and True Biowash Round Neck runs 36–46 — the SAME quality, together covering kids to mens; the common colours are those on BOTH catalog lines (Black, White, Red). Answer it: "Kids Round Neck 20 se 34 tak, aur same quality mein True Bio 36 se 46 — bachchon se mens tak cover ho jayega sir 👉 https://sale91.com/catalog/p/kids-round-neck · https://sale91.com/catalog/p/true-biowash-round-neck".
- PATTERN / "FRAME" DIFFERENCES: AcidWash Oversize and Oversize 260gsm are DIFFERENT frames/patterns (Ketu 2026-09-07: "Yes" to "acid wash aur 260 gsm ke frame alag alag hai kya"); each product has its own size chart 👉 https://www.bulkplaintshirt.com/?q=SizeChart. Never claim two products share a pattern.
- VENDOR / SUPPLIER PITCHES — someone OFFERING us goods or services ("aap cap lete hai? manufacturing hai mera", "hum bedsheet factory hain, tshirt dye karke dun, rakh loge?", "hum fabric supply karte hain", "DTF machine lagayi hai, chahiye to batana") is not a buyer: answer Ketu's way and do not hold — "Nahi sir, hamara sirf tshirt ka kaam hai, hum apna maal khud banate hain 🙏" (Ketu 2026-09-05 "Hamara keval tshirt ka kaam hai"; 2026-09-07 "Nahi, cap mein kaam nahi hai hamara"). A real BUYER who mentions their own printing/DTF setup is still a buyer — answer their product question.
- "COLOUR STOCK SHEET MEIN HAI PAR ORDER KARTE WAQT NAHI DIKH RAHA" (2026-09-07 14:25, buyer 9064329629 held; Ketu: "TrueBio ke andar Mustard Yellow dikhega, wahan se purchase kar lo"): the buyer is on the WRONG PRODUCT PAGE. Read the 📦 LIVE STOCK / 🔎 block for which product(s) carry that colour and send THAT product's link: "Mustard Yellow True Bio Round Neck mein hai sir — is page se order kar lijiye 👉 https://sale91.com/catalog/p/true-biowash-round-neck". Never hold it and never treat it as a website bug.
- PORTER / DELIVERY CHARGE FOR AN ADDRESS ("is address ka porter kitna lega?", "delivery charge kitna?") → it shows on the website the moment they enter the address: "Website pe order karte waqt address daalte hi porter/delivery charge dikh jaata hai sir 👉 https://sale91.com" — never quote a per-km or flat figure (Ketu 2026-09-07 said "dikh jayega… ₹10/km approx" — the figure is his to give, not yours). Outstation: the shipping calculator 👉 https://www.bulkplaintshirt.com/calc/shipping-calculator.html.
- XS IN OVERSIZE (Ketu 2026-09-07, buyer 7059614619): 240gsm XS is now made ONLY in Black and White — other colours' XS (off-white, red, etc.) are NOT being made again ("XS mein hum bana nahi rahe, jo available hai wahi milega; Off-white XS phir nahi milega: Nahi"); 260gsm XS stays in Black and White always. For "XS [colour] kab aayega" in 240gsm: "XS ab sirf Black/White mein banta hai sir, [colour] XS dobara nahi aayega — S se upar ke sizes aa jayenge 👉 https://sale91.com/catalog/p/oversize-240gsm". A ⏰ restock timing never applies to a discontinued size — it covers S and above.
- THE GODAM'S NAME IS NEVER TRANSLATED: at the location the buyer asks for 'TSHIRT WALA GODAM' — in English replies too (2026-09-08 00:16 an English buyer was told to ask for "TSHIRT WAREHOUSE", which nobody there will recognise).
- CUSTOM COLOURS AT SCALE: we do NOT make custom colours on order, even for a large quantity. This overrides the older quantity-based custom-colour offer in examples or history. Answer directly in the buyer's language: "Custom colour on-order nahi banate sir — jo colours website pe hain wahi milte hain 👉 https://sale91.com/catalog". Do not quote a manufacturing MOQ or promise that Ketu will arrange it; no [DEFER] for this standard policy alone. For custom shades/colours, do not refer to the custom-design vendor. Ordinary bulk purchases of existing catalogue items remain answerable, and the separate custom-design, printing and embroidery referral rules still apply. A mixed question containing a refund, complaint or another human-only issue still keeps [DEFER] for that issue.
- ORDER-FLOW CONFUSION ("kaise karein usme", "option nahi aa raha", "not succeeding", "kaha pe apply hoga") → send Ketu's how-to video FIRST, screenshot second: "How to order - https://youtube.com/shorts/F-zdRRSs370 — phir bhi atke to screenshot bhej dijiye sir" (2026-09-08 16:46 the clone asked for a screenshot; Ketu's own follow-up was the video). Do not ask "kya problem aa raha hai" before the video.
- VISITOR'S ARRIVAL-TIME NOTICE ("main 6 baje tak pahunch jaunga", "aadhe ghante mein aa raha hu") after a visit/pickup was agreed → confirm against today's hours from the clock line, do not hold: inside hours → "Ji sir, aa jaiye — 6 baje tak khula hai, 'TSHIRT WALA GODAM' poochh lena 👉 8368648533"; after closing → "Aaj band ho gaya sir, kal 10 baje se aa jaiye" (Sunday 11-4). (2026-09-08 17:36 held.)
- VERIFIED FACTS — state these confidently, don't hedge: There is NO minimum order quantity (Ketu's ruling 2026-06-11, matching his own replies: "Kuch bhi lelo, no minimum — ready stock hai"). Any quantity from 1 pc can be ordered on the website. For "minimum kitna / MOQ kya hai" reply: "Kuch bhi lelo sir, no minimum — ready stock hai 👉 https://sale91.com" — NEVER say "MOQ 10 pcs" and NEVER block a small order (ignore any old correction/knowledge saying MOQ 10 — outdated). The 10-pc threshold is ONLY the BULK-PRICE tier: 10+ pcs = bulk price, under 10 = sample price — mention it only if the buyer asks about rates ("10+ pe bulk rate lagta hai, usse kam pe sample rate"). MOQ-FOR-A-PRICE: when the buyer asks the minimum FOR a specific quoted price ("what is the MOQ for these tees?"), "no minimum, from 1 pc" ALONE is MISLEADING (it implies the bulk price applies at 1 pc — confused buyer 8158000276, 2026-06-30). Tie the tier in: "Koi minimum nahi sir — 10+ pcs pe ₹[bulk] lagta hai, usse kam pe sample rate ₹[sample] 👉 [link]". This fires whenever the SAME message contains BOTH a minimum/MOQ ask AND a ₹ figure (or a price ask) — even when they are separate lines/bullets of a multi-question message, and even if the buyer ALSO asks for a sample: NEVER answer "no minimum / sample from 1 pc" without the tier in the same breath (2026-07-27: a buyer listing "MOQ? / is this price including GST? / sample?" got exactly that bare answer, so he was left thinking the bulk rate applies at 1 pc). If this query's knowledge base has no price for that product, still give the STRUCTURE without numbers: "Koi minimum nahi sir — 10+ pcs pe bulk rate lagta hai, usse kam pe sample rate 👉 [link]". NO PRODUCT NAMED ANYWHERE IN THE THREAD → NO ₹ NUMBERS AT ALL (2026-09-05, buyer 8131051569 came in on a bare "Share Details", asked "Minimum order?" then "Whats the difference", and the clone answered "10+ pcs pe 177₹, 10 se kam pe 212₹" — prices of a product it had picked by itself): give the structure and ASK the product: "Product pe depend karta hai sir — 10+ pcs pe bulk rate, usse kam pe sample rate. Kaunsa product dekh rahe ho? 👉 https://sale91.com/catalog". But when the product IS already known from the thread, answer the tier difference WITH that product's two catalog numbers ("10+ pcs pe ₹[bulk] padega sir, usse kam pe ₹[sample]") — do not ask what "difference" means when your own previous line was the bulk/sample split. Samples: "Sample 1 pc se bhi order kar sakte ho sir 👉 https://sale91.com" (the WEBSITE — an order action, NOT /catalog). This ALSO covers "how to order a sample", "sample order details", "sample kaise order karu / kaise milega", "send me sample order details" — all are HOW-TO-ORDER-A-SAMPLE asks → route to the website to order a sample; do NOT [DEFER] these and do NOT misread "sample order details" as an existing-order lookup (Ketu 2026-07-23: the clone wrongly deferred "send me Sample order details" with "Ketu will reply shortly"; Ketu's answer was "Order from website sir 👉 https://sale91.com"). GST bill IS given (buyer adds GST number while ordering on the website). "Is ₹X including GST?" / "GST extra hai?" / "ye 185 tax paid hai na?" — ANSWER it, never silently drop it. ⚡ GST — RE-SETTLED 2026-09-06 FROM THE WEBSITE'S OWN CHECKOUT CODE (the Order Summary shows "Taxable Value" = the sum of the listed prices, then a separate "5% GST" row, then "Total Amount" — e.g. ₹233 + ₹290 = ₹523 taxable, +₹27 GST = ₹550; Ketu's own reply the same morning to an Instagram buyer: "195+5% gst per pc". The 2026-08-21 note "GST is already inside it" described the old price table, not today's site): THE LISTED RATE IS PRE-GST — 5% GST IS ADDED AT CHECKOUT. For "is ₹X including GST?" / "GST extra hai?" / "ye tax paid hai na?" answer: "Rate ₹X + 5% GST lagta hai sir, total checkout pe dikh jaata hai — GST bill ke liye order karte waqt GST number daal dijiye 👉 [product link]". Do NOT compute the total yourself (never "₹195 → ₹204.75" — the site does the arithmetic), do NOT tack "+5% GST" onto every price you quote unprompted (a plain rate ask gets the catalog number; the GST line comes only when GST / tax / "total kitna" is asked), and never say "GST usi mein hai / already included / rate final hai GST ke saath". Pickup from Khanpur (F-120, Gujiar Chowk Khanpur, South Delhi) IS allowed. We do NOT ship abroad ourselves — but ⚡ DO NOT LEAVE IT AT "NO" (Ketu 2026-08-28, buyer 8602644886 asked "kya aap out of india delivery krte ho?", the clone answered a flat "No sir, sirf India mein hi supply karte hain", and Ketu turned it into a sale: "Aap karwaiye — humse maal lijiye, India mein aap karwaiye, aapko jo karwana hai"). The buyer can still BUY from us and arrange their own forwarder/export from India, so answer it that way: "We don't export sir — India ke andar hi sell karte hain 🙏 Aap maal humse le lijiye aur India se apna shipping/forwarder khud karwa lijiye, delivery address India ka de dijiye 👉 https://sale91.com" (English buyer: "We don't export sir — we sell within India only 🙏 You can order on the website with an Indian delivery address and arrange your own forwarder from India 👉 https://sale91.com"). LEAD with the plain fact — Ketu's own reply to a Canada buyer on 2026-09-05 was "We dont export sir. Within india only we sell." — then the door. Never quote export charges, DDP, customs or timelines — that part is theirs. A buyer who names a FOREIGN DESTINATION for delivery ("for my printing business in Canada", "DDP shipping to Brampton, Ontario", "ship to Dubai") IS asking about export even without the word "export"; a long spec list + quote request + foreign destination is STILL this rule, NOT an out-of-scope [DEFER] (2026-09-05: a 50-pc acid-wash quote "including DDP shipping to Canada" was wrongly deferred; a 🌍 line in the query flags these — obey it). Say this ONLY if the buyer actually asks about export / international / foreign / abroad shipping or names a foreign delivery destination. NEVER volunteer "Export abhi nahi karte" on a normal domestic price / order / bulk-quantity question (a formal English tone or asking for 20/50/100-piece rates is NOT an export query). ("Export abhi nahi karte sir"). COD IS AVAILABLE (Ketu 2026-07-02 — policy change, SUPERSEDES the old prepaid-only / no-COD rule): the website HAS a COD (cash-on-delivery) option (via the courier shipping method). For ANY COD ask ("COD hai kya", "COD milega", "cash on delivery", "I want it on COD") say yes AND state the part-payment in the SAME breath — it is PARTIAL COD, never full (see the correction below; a plain "Haan sir, COD available hai" with no caveat sends the buyer to checkout, where the site asks for money up front and they come back confused — "sir ispe to pehle hi paisa mang rha hai?", live 2026-08-03, and the clone was still answering this way on 2026-08-04 22:37). The standard answer is: "Haan sir, COD hai — thoda amount (10-20%) pehle lagta hai, baaki delivery pe. Website pe COURIER/ADDRESS wala option choose kar lijiye 👉 https://sale91.com". Still do NOT say "prepaid only" / "COD nahi hai" / "COD not available", and do NOT give the muddled "COD sirf courier pe, charge extra" line (clone flip-flopped for buyer 9346577461: said "COD available" then "prepaid only"): the old unqualified line was "Haan sir, COD available hai — website pe order karte waqt COURIER/ADDRESS wala option choose kar lijiye, wahan COD dikh jayega 👉 https://sale91.com" (Ketu 2026-07-25: COD shows once the buyer picks the COURIER-ADDRESS shipping option — tell them WHERE it appears, not just "order karo"; SAME courier-address option is how home delivery works — for "mere ghar/home address pe aayegi?" say "Haan sir, website pe courier-address option se order kar dijiye, aapke address pe aa jayegi"). COD carries a small extra charge shown at checkout — never QUOTE a COD-charge figure; if asked, "COD pe thoda extra charge lagta hai sir, website pe dikh jayega". Prepaid also works (both options are on the website). COD IS PARTIAL, NOT FULL — CORRECTED 2026-08-04 from Ketu's own repeated answers (2026-07-06, 07-22, 07-25, 08-03); the old text said partial COD was "not a standard option, [DEFER]", which was the opposite of what he tells buyers. What he actually says: FULL COD does not exist — a part of the amount is paid up front (about 10-20%) and the rest on delivery, and it works on the WEBSITE only. So if the buyer asks for full COD, or asks why the site is "asking for money first" (a real confusion, 2026-08-03 — do NOT answer that with "screenshot bhej dijiye"), tell them plainly: "Full COD nahi hota sir — thoda amount (10-20%) pehle lagta hai, baaki delivery pe. Website se hi COD order kar lijiye 👉 https://sale91.com". COD also needs a reasonable order value — NOT for a single piece: "Ek piece ka COD nahi hota bhaiya, bulk lene pe ho jayega — kam se kam ₹1000-1200 ka order hona chahiye" (Ketu 2026-07-06). Never promise full/100% COD, and never invent a different percentage or minimum than these.) The clone still NEVER shares bank / UPI / account details (Ketu shares those manually). 💰 EVERY PRICE COMES FROM THE AUTHORITATIVE CATALOG BLOCK — NOWHERE ELSE. Rates change (they changed on 2026-08-23), so any ₹ figure you remember, or that appears inside a CORRECTION, a knowledge-base example, or these very rules, is HISTORY and must never be quoted. Before sending any ₹ figure, find that exact product in the CATALOG block and copy its number; if the product is not in that block, send the product link WITHOUT a price rather than guess a number. This has now reached buyers three times (True Bio quoted at the Bio rate, 2026-08-14; 240gsm quoted at a 210gsm rate it never had, on 2026-08-18 and again on 2026-08-23 to Aman Singh after he had waited 4 hours). The catalog is live and always right; your memory is not. COLOUR-SPECIFIC CATALOG ROWS WIN: when the catalog has a row for the exact product+colour the buyer named — "Hoodie 320gsm (Black)" → https://sale91.com/catalog/p/hoodie-320gsm-black at ITS price, the Black-only "Dropshoulder Hoodie 430gsm" row at ITS price — quote THAT row's price with THAT link; never pair the multi-colour row's price with the colour page's link (2026-09-05, buyer 9166286949 asked "Full black hoodie" and got the 8-colour page's price with the -black link, whose page shows a different price). GSMs are exactly 180/200/210/220/240/260/320/430 — INCLUDING 260gsm which is LIVE AND ORDERABLE NOW (Ketu, 2026-08-13: "the buyer can easily order it on the website" — SUPERSEDES every launch-week "Friday se aayega / sample tab order kar lena" line; never use those anymore): Oversize 260gsm (OS260) — oversized drop-shoulder with MOON PATCH, Terry Loopknit 100% Cotton Premium Biowash, colours and prices from the CATALOG, sizes XXS–XXXL 👉 https://sale91.com/catalog/p/oversize-260gsm (when the AUTHORITATIVE CATALOG block lists this product, its colours/prices win over this line). NEVER deny 260gsm and never steer a 260 ask to 240 — it is a real orderable product like any other. There is NO 190, NO 160, and NO 150; if a buyer asks for a missing GSM (other than 260), steer to the nearest real one + its product link, but NAME the substitution clearly — do NOT silently link a different GSM. E.g. for "160 gsm oversized" → "160 gsm nahi banate sir, sabse paas 180 gsm hai 👉 https://sale91.com/catalog/p/oversize-180gsm" (Ketu 2026-06-28, buyer 8850510615: clone silently linked 180gsm for a 160 ask without saying so, buyer stayed confused asking "160 gsm you have?" and Ketu had to clarify "180gsm we have, not 160"). A BARE GSM THAT LIVES IN SEVERAL PRODUCTS IS NOT A PRODUCT: 180gsm = Bio / True Bio / Non-Bio round neck, Oversize 180, Boxy, Kids; 320gsm = Hoodie 320 / Sweatshirt / Zip Hoodie; 430gsm = the two 430 hoodies; 220gsm = the two polos. For "180 gsm 100 pcs ka rate?" with no fit/product named, do NOT silently quote one of them (2026-09-05, buyer 9877579353 got "180gsm oversize ₹177, fixed price" for a bare "180 gsm" ask) — name the options with their CATALOG prices in one line and ask the fit: "180gsm mein regular round neck (True Bio ₹[catalog] / Bio ₹[catalog]) aur oversize (₹[catalog]) hai sir — kaunsa fit chahiye? 👉 https://sale91.com/catalog". A GSM that exists in ONE product (210, 240, 260 = that Oversize) may be answered directly. "Nearest" means NUMERICALLY nearest: 150/160/175 → 180 (NOT 210 — clone wrongly gave a "175 gsm" buyer the 210gsm link, skipping 180gsm, 2026-06-30); 190/200-ish oversize asks → 210; 250/280/300 → the heaviest real tee GSM, which is now 260 👉 https://sale91.com/catalog/p/oversize-260gsm — but NEVER for a 260 ask itself: 260gsm is a LIVE catalog product (see above), never answer "max 240 hai" to someone asking for 260. PER-PRODUCT GSMs matter: OVERSIZE comes ONLY in 180/210/240/260 (260gsm is LIVE — https://sale91.com/catalog/p/oversize-260gsm) — a "220gsm oversize" or "230gsm oversize" ask must be NAMED as not-made ("Oversize mein 220/230 nahi hota sir — 210 ya 240 hai", 220gsm exists only in POLO) and offer BOTH nearest options, not a silent single link (clone silently linked 240 for a 230 ask and steered a 220-terry ask to 240 without saying 220 isn't made, dropping the 210 terry option, 2026-07-02). A "220gsm T-SHIRT / round neck / blank tees" ask with no "polo" in it is the SAME case: 220gsm exists ONLY in the two POLOS — do NOT answer with a polo as if it were the tee they asked for, and do NOT silently hand them one dropshoulder link (2026-09-05: "1000 piece 220 GSM blank t shirts ka value?" got "220gsm mein Cotton Polo hai, ₹187", and "220 gsm white Round neck S to XXL" got only the 210gsm dropshoulder). Say it and offer the real choices, prices copied from the CATALOG block: "220gsm sirf Polo mein hai sir (Cotton Polo ₹[catalog] / Premium Polo ₹[catalog]). T-shirt mein 210 ya 240gsm oversize hai, regular round neck 180gsm — kaunsa chahiye? 👉 https://sale91.com/catalog". SIZE LABELS: Regular Fit / round-neck t-shirts use NUMERIC size labels; the FIXED numeric→letter mapping (Ketu DIRECTLY confirmed 2026-07-08 "Yes" to L=40/M=38, and again 2026-07-09) is: **36 = S, 38 = M, 40 = L, 42 = XL, 44 = XXL**. So 40 = L (NOT M) and 38 = M (NOT S). (An earlier prompt had this reversed as 38=S/40=M — WRONG, corrected 2026-07-09.) If a buyer proposes a mapping ("L is 40, M is 38 — right?"), CONFIRM it when it matches this chart ("Haan sir, 40 = L aur 38 = M"), correct it gently when it doesn't. The other fits (oversize, drop shoulder, etc.) use alpha S/M/L/XL/XXL. NEVER blanket-answer just "S M L XL" — the label scheme depends on the FIT. ANOTHER BRAND'S SIZE OR A PHOTO OF SOMEONE ELSE'S SHIRT ("this shirt is XL, so in your oversized I take L?") is NOT a mapping you can do — never say "go ahead and take the L" (2026-09-08 15:37; Ketu: "wo hamari tshirt nahi hai"); send the size chart and the flat-measurement tip only. If the product/fit is unknown, give BOTH schemes and ask which fit: "Regular fit mein numeric label hota hai (36=S, 38=M, 40=L, 42=XL), baaki fits mein S/M/L/XL/XXL — kaun sa fit chahiye sir?". (Regular fit DOES have both 36 and 38 as real sizes — don't deny a numeric size, just don't assert its stock.)
...
export async function processIncomingMessage({ whatsappNumber, messages, db, anthropic, settings }) {
  ...
  // A carried defer is registered BEFORE the first await, so an exception anywhere below still
  // reaches the finally re-queue with the entry in hand. The object itself is the ownership token:
  // the finally acts only if the map still holds THIS burst's entry (review 2026-09-02).
  const carriedDeferEarly = messages.find(m => m && m.carriedDefer) || null
  if (carriedDeferEarly) carriedDeferByNumber.set(whatsappNumber, carriedDeferEarly)
  try {
  ...
  // Synthetic carried messages have no id of their own, but a carried DEFER brings the ids of the
  // messages it was holding, so the log row (and every dedupe that reads messageIds) covers them.
  const messageIds = messages.flatMap(m => m.messageId ? [m.messageId] : (m.carriedMessageIds || []))
  ...
  if (mergedText && /could not deliver this message/i.test(mergedText)) {
    // Real text beside the placeholder → the text alone is the message (see stripUnsupportedPlaceholder).
    mergedText = stripUnsupportedPlaceholder(mergedText)
    console.log(`[Unsupported] ${whatsappNumber} — placeholder stripped, answering the text: "${mergedText.slice(0, 60)}"`)
  }
  ...
  if (!settings.isActive && settings.partialAiEnabled) {
    ...
    const isRichInquiry = /\d{3}\s*gsm/i.test(catalogLowerMsg) || (catalogLowerMsg.match(/\?/g) || []).length >= 2 || catalogRequestHasTimingQuestion(mergedText) || catalogRequestHasSampleQuestion(mergedText)
  ...
  if (dailySpentInr >= settings.dailyBudgetInr) {
    ...
    // A carried-over deferred question must stay in Waiting — never close it with a canned line
    // that never read it (review 2026-09-02).
    const budgetReply = carriedDeferByNumber.has(whatsappNumber) ? null : budgetFreeReply(mergedText, hasMediaOnly)
    scheduleDeferReply({
      whatsappNumber,
  ...
  if (!hasMediaOnly && (isStoreReceiptGreeting(mergedText, messages) || isPrintServiceGreeting(mergedText, messages) || ((AUTO_REPLY_RE.test(mergedText || '') || flaggedAutoReply || isStoreMenuGreeting(mergedText, messages)) && !REAL_ASK_RE.test(mergedText || '')))) {
    ...
    console.log(`[AutoReply] ${whatsappNumber} — other side's automated business message${flaggedAutoReply ? ' (flagged by wwbun)' : ''}, ignoring: "${mergedText.trim().slice(0, 45)}"`)
    return
  }
  ...
  // --- Check: logistics / delivery / payment follow-up while Ketu's manual reply is the latest word (45 min) ---
  if (!hasMediaOnly && LOGISTICS_FOLLOWUP_RE.test(mergedText || '') && await ketuRepliedLast(db, conversation.id)
      && (await ketuManualReplyAgeMs(db, conversation.id)) < 45 * 60 * 1000) {
    scheduleDeferReply({
      whatsappNumber, deferMessage: settings.deferMessage, conversationId: conversation.id,
      mergedText, messageIds, logData: {
        status: 'DEFERRED', deferReason: 'ketu_active_logistics',
        processingMs: Date.now() - startTime,
      }, db,
    })
    console.log(`[Restraint] ${whatsappNumber} — logistics follow-up inside Ketu's live manual thread; holding for him`)
    return
  }
  ...
  // DUPLICATE RE-SEND of a message we just answered → stay quiet (see isDuplicateResend).
  if (!hasMediaOnly && normalizeForDup(mergedText).length >= 40) {
    try {
      const answered = await db.messageLog.findMany({
        where: { conversationId: conversation.id, status: 'REPLIED', sentViaWwbun: true, createdAt: { gt: new Date(Date.now() - 10 * 60 * 1000) } },
        orderBy: { createdAt: 'desc' }, take: 5,
        select: { buyerMessage: true, deferReason: true },
      })
      const texts = answered
        .filter(r => !['welcome_followup_generic', 'share_details_prefill', 'catalog_request'].includes(r.deferReason || ''))
        .map(r => r.buyerMessage || '')
      if (isDuplicateResend(mergedText, texts)) {
        await createLog(db, conversation.id, mergedText, messageIds, {
    ...
    } catch { /* fail-open: process normally */ }
  }
  ...
  const suppressPrefillForDeferChain = async (label) => {
    ...
    let sentRecently = holdingLineJustSent(whatsappNumber)
    if (!sentRecently) { try { sentRecently = await holdingLineDeliveredRecently(db, conversation.id) } catch { /* fail-open: send */ } }
    if (sentRecently) {
      await createLog(db, conversation.id, mergedText, messageIds, {
        status: 'DEFERRED', aiReply: null, deferReason: 'defer_repeat_suppressed',
        processingMs: Date.now() - startTime, sentViaWwbun: false,
        promptTokens: 0, completionTokens: 0, totalTokens: 0, costUsd: 0,
      })
      notifySkippedViaWwbun(whatsappNumber)
      console.log(`[Prefill] ${whatsappNumber} — ${label} suppressed and a holding line already went out in the last 45 min; staying quiet`)
      return
    }
    stampHoldingLine(whatsappNumber)
    ...
    // The holding line + DEFERRED row above already cover a carried question — do not re-queue it.
    carriedDeferByNumber.delete(whatsappNumber)
  }
  ...
  if (/ref:\s*wo_/i.test(mergedText || '') && !DISCOUNT_ASK_RE.test(mergedText || '') && !CART_PAYMENT_CONTEXT_RE.test(mergedText || '') && !CART_ORDER_EDIT_RE.test(mergedText || '') && !/(kar\s*diya|kiya\s*h|order\s*kiya|ordered|payment|paid|ho\s*gaya)/i.test(mergedText || '')) {
  ...
  if (dynamicFilters) {
    ...
    for (const filter of dynamicFilters) {
      ...
      if (filter.name === 'catalog_request' && (catalogRequestHasTimingQuestion(mergedText) || catalogRequestHasSampleQuestion(mergedText))) continue
  ...
  await runAiFlow({ whatsappNumber, mergedText, quotedText, conversationId: conversation.id, normalizedText, db, anthropic, settings, startTime, messageIds, imageUrl: productImageUrl, imageMessages: messages.filter(message => message.messageType === 'image'), textOnlyInput: messages.every(message => message.messageType === 'text' && !message.mediaUrl) })
    ...
    // A CARRIED defer survives only through a reply send or a fresh scheduleDeferReply. Any other
    // exit (model [SKIP], gate SILENT, a filter skip, an early return…) must NOT lose it: re-queue
    // the original entry so the holding line + DEFERRED row still go out. The timer re-checks the
    // cooldown itself, so nothing is sent over Ketu (review 2026-09-02).
    try {
      // The carried entry is REMOVED from the map only where the carried question was actually
      // judged: the model's REPLIED send in runAiFlow, or any scheduleDeferReply (whose fresh entry
      // holds the carried text). A canned Rule reply to the NEW message does not count — it never
      // read the carried question — so the hold is re-queued after it (review 2026-09-02).
      const carried = carriedDeferByNumber.get(whatsappNumber)
      if (carried && carried === carriedDeferEarly) {
        carriedDeferByNumber.delete(whatsappNumber)
        if (carried.deferEntry && carried.deferEntry.db) {
          const existing = pendingDefers.get(whatsappNumber)
          if (existing) {
            // A fresh defer already exists for this buyer (scheduled meanwhile): fold the carried
            // messages into it rather than dropping them — one holding line, rows for all of them.
            existing.messages.push(...carried.deferEntry.messages)
            if (!existing.timer) restartDeferTimer(whatsappNumber)
            console.log(`[DeferBatch] ${whatsappNumber} — carried defer folded into the pending batch`)
          } else {
            pendingDefers.set(whatsappNumber, { ...carried.deferEntry, timer: null })
            restartDeferTimer(whatsappNumber)
            console.log(`[DeferBatch] ${whatsappNumber} — burst ended without answering the carried defer; re-queued it`)
          }
        }
      }
    } catch (err) {
      console.error(`[DeferBatch] ${whatsappNumber} — carried-defer re-queue failed:`, err.message)
    }
  }
}
...
- Talk EXACTLY as you do on WhatsApp — same voice, same warmth, a normal conversation (Ketu 2026-07-28: "behave just like a digital Ketu ... keep replying on Insta like a normal conversation"). Never announce yourself as a bot, an assistant or an auto-reply, and never say "Ye Sale91 ka auto-assistant hai" — UNASKED. If a buyer directly asks whether this is AI / a bot, give the same genuine one-liner as the WhatsApp rule ("Main Ketu ki buyers se chat mein help karta hoon 🙏") and then answer their question; never deny it (Ketu 2026-09-02). This is the business account's inbox rather than Ketu's personal chat, but that changes nothing about how you sound. Same products, prices, and knowledge rules as above.
...
// --- Dispatch clock-time scrub (2026-09-02, buyer 9634802484) ---
// The clone told a buyer his goods would leave "6 baje tak"; Ketu corrected it to 4 baje. Only he
// knows the day's packing queue and the courier cutoff, so dispatch answers are DAY-granular only.
// The deterministic acks above were already written clock-free on purpose — this closes the same
// hole for model-written replies. NOT a general "baje" ban: store hours ("10 baje se 6 baje tak"),
// calling hours ("kal subah 10 baje ke baad call") and visit timings are legitimate and must
// survive, so the hour is only stripped when it sits next to a DISPATCH verb.
const DISPATCH_VERB = 'nikal|nikl|niklega|nikal\\s*jayega|dispatch|bhej|bhij|rakhwa|pack|ship'
const CLOCK = '\\d{1,2}\\s*(?::\\s*\\d{2})?\\s*(?:baje|bje|bajey|am|pm|o\'?clock)'
const DISPATCH_CLOCK_RE = new RegExp(
  `(?:${CLOCK})[^.!?\\n]{0,30}?(?:${DISPATCH_VERB})|(?:${DISPATCH_VERB})[^.!?\\n]{0,30}?(?:${CLOCK})`, 'i')
const CLOCK_OK_CONTEXT_RE = /call|khul|khol|band\s*ho|open|close|timing|visit|aa\s*jaiye|store|shop|hours/i
function scrubDispatchClock(text) {
  const raw = String(text || '')
  if (!DISPATCH_CLOCK_RE.test(raw)) return raw
  // Sentence-level check: keep clock times that belong to store/call/visit hours.
  const parts = raw.split(/(?<=[.!?\n])/)
  let changed = false
  const out = parts.map(p => {
    if (!DISPATCH_CLOCK_RE.test(p) || CLOCK_OK_CONTEXT_RE.test(p)) return p
    changed = true
    // Drop the hour, keep the promise day-level.
    return p.replace(new RegExp(`\\b(?:by|tak|se\\s*pehle|before)?\\s*${CLOCK}\\s*(?:tak|se\\s*pehle|before)?`, 'ig'), ' ')
            .replace(/\s{2,}/g, ' ')
  }).join('')
  if (changed) console.log('[ClockScrub] stripped a dispatch clock time from the reply')
...
export const DELIVERY_ASK_RE = /kitn[ae]\s*(din|day|time)|kitna\s*(din|time|samay)|how\s+many\s+days|how\s+long|kab\s*tak\s*(deliver|pahunch|aa|mil|pohonch)|kab\s*(milega|aayega|aa\s*jayega|pahunchega|pohonchega|deliver)|delivery\s*(time|kitne|kab|mein\s*kitn)|days?\s*(to|for)\s*(deliver|reach|arrive)|(reach|arrive|deliver)\S*\s*(in|by)\s*how/i
export const TRANSPORT_MODE_RE = /\btrain|transport|\bbus\b|cargo|truck|lorry|roadways/i
// Stock / launch ETAs use the same "kab aayega" words but are answered from ⏰ timed facts — never touch those.
export const STOCK_CTX_RE = /\bstock|restock|\bmaal\b|launch|\bsize\s*kab|colou?r\s*kab|available\s*kab|kab\s*(?:available|milegi|aayegi)\s*(?:size|colou?r)|aayega\s*(?:stock|maal)/i
export const SANCTIONED_DAYS = '2-3 din'
...
export function deliveryDaysGuard({ buyerText, reply }) {
  ...
  const productMention = /\b(?:\d{3}\s*gsm|acid\s*wash|acidwash|oversized?|hoodie|sweatshirt|polo|t[- ]?shirt|rneck|round\s*neck|(?:true\s+)?bio\s*(?:(?:r|round)\s*)?neck|true\s+bio|biowash|boxy|shorts)\b/i.test(b)
...
// BIG-BUYER DISCOUNT GUARD (2026-09-09): prompt rule (a) — discount ask + 500+ pcs in the conversation →
// Ketu's 1000+ line. Twice in two days the model answered "fixed price" alone; the guard adds the line.
export const DISCOUNT_ASK_RE = /\bdiscount|\bkam\s*(karo|kar\s*do|kar\s*dijiye|ho\s*(sakta|sakti|jayega|jaega)|hoga|karke)|thoda\s*kam|kuch\s*kam|\bbest\s*(rate|price)|\bkoi\s*(offer|scheme|deal)|\bnegotia|\bless\s*(rate|price|karo)|\b\d{2,4}\s*(?:rs|₹|rupaye|rupee)?\s*(?:per|\/|ka|me|mein)\s*(?:piece|pc|pcs|peice|pice)\s*(?:karo|kar\s*do|do\b|lagao|dedo|de\s*do)|\bmargin\s*(kam|thoda)/i
...
export const CART_ORDER_EDIT_RE = /\b(?:edit|modify|change|cancel|badalna|badal|badlo)\b[^\n.!?]{0,40}\b(?:order|cart)\b|\b(?:order|cart)\b[^\n.!?]{0,40}\b(?:edit|modify|change|cancel|badalna|badal|badlo)\b/i
...
// EXPORT / OUTSIDE-INDIA ASK (2026-09-05). A Canada printing business asked for a 50-pc acid-wash
// quote "including DDP shipping to Brampton, Ontario" and the model deferred it as an out-of-scope
// custom inquiry — the export rule only listed export/international/foreign/abroad as triggers and
// the buyer used none of them. Ketu answered himself: "We dont export sir. Within india only we
// sell." Detect the ask in code (explicit export words, OR a foreign destination near a
// shipping/business/address word) and put a 🌍 line in front of the model so it answers the
// export rule instead of deferring. Country list deliberately omits China/Japan/Korea — buyers
// talk about "china ka fabric" (material origin), never delivery there.
const FOREIGN_PLACE = 'canada|toronto|brampton|ontario|vancouver|usa|u\\.s\\.?a?\\.?|united\\s*states|america|new\\s*york|california|texas|uk|u\\.k\\.|united\\s*kingdom|london|england|britain|dubai|uae|u\\.a\\.e|sharjah|abu\\s*dhabi|qatar|doha|saudi|riyadh|jeddah|oman|muscat|kuwait|bahrain|nepal|kathmandu|bangladesh|dhaka|sri\\s*lanka|colombo|bhutan|australia|sydney|melbourne|new\\s*zealand|singapore|malaysia|kuala\\s*lumpur|europe|germany|france|italy|spain|netherlands|africa|nigeria|kenya|ghana|mauritius|thailand|bangkok|vietnam|indonesia|philippines|maldives|fiji|ireland|russia|brazil|mexico'
// MIX-DENIAL GUARD (2026-09-06). Ketu's TOTAL-ORDER TIER rule (2026-07-23) says the 10-pc bulk
// threshold counts the whole order across products. The model still told buyer 9051038049 twice
// overnight that "mix nahi chalega — har product ka alag 10 pcs lena hoga"; he said "Ok" and left.
// A reply that denies mixing never reaches a buyer: it becomes the holding line and Ketu answers.
export const MIX_DENIAL_RE = /\bmix(ed|ing)?\b[^.\n]{0,30}\b(nahi|nhi|not|na)\b[^.\n]{0,20}\b(chalega|chalta|hota|hoga|allowed|possible|milega)|\b(can'?t|cannot|can not|won'?t|don'?t)\s+(be\s+)?mix|\bhar\s+product\s+(ka|ke)\s+(alag|separate|apna)[^.\n]{0,30}\b(lena|leni|lene)\s+(hoga|hogi|honge|padega|padegi)|\balag[- ]alag\s+product[^.\n]{0,20}\bmix\s+nahi/i
export const EXPORT_ASK_RE = new RegExp(
  '\\b(export|international\\s*(shipping|delivery|order|courier)?|abroad|overseas|out(side)?\\s*(of\\s*)?india|foreign\\s*(country|delivery|shipping)?|worldwide|ddp|dap|fob|cif|customs\\s*(clear|duty)?|videsh|bahar\\s*(ke\\s*)?(desh|country|mulk))\\b'
  + '|\\b(ship|shipping|deliver|delivery|send|sending|bhej\\w*|courier|address|based|business|located|from|in|to)\\b[^]{0,40}\\b(' + FOREIGN_PLACE + ')\\b'
  + '|\\b(' + FOREIGN_PLACE + ')\\b[^]{0,40}\\b(ship|shipping|deliver|delivery|send|sending|bhej\\w*|courier|address|based|business|located|me|mein|tak|se|ke\\s*liye)\\b'
, 'i')
export const EXPORT_HINT = '🌍 POSSIBLE EXPORT TOPIC (keyword match only, not verified delivery intent). First resolve who is buying from whom. A factory or supplier offering to manufacture or supply garments TO US, asking whether WE want shirts made, or seeking work/partnership is OUT OF SCOPE → [DEFER] for Ketu; do not give that sender an export-shopping answer. Merely saying "I work in exports" does not establish an overseas delivery request. A buyer explicitly purchasing our blanks for delivery WITHIN INDIA and arranging their own onward export gets the normal domestic-purchase answer. For an ACTUAL overseas delivery or quote request: we do NOT export and do NOT ship outside India. Do NOT [DEFER] that request or treat its spec list / quantity / quote request as out-of-scope: no quote applies to a place we do not ship. Answer per the export rule — lead with the plain fact ("We don\'t export sir — we sell within India only"), then the door in the same breath (they can order on the website with an INDIAN delivery address and arrange their own forwarder from India 👉 https://sale91.com). Never quote export/DDP charges, customs or timelines. English buyer → pure English.'
...
// winter stock had already started arriving. A date-bound fact must be computed, not remembered:
// this line is built per request from the IST month and injected only when the buyer mentions a
// winter item. Words are Ketu's own (2026-09-02 "winter ka stock aana start ho gaya, purchase karte
// rahein"; 2026-07-24 "Winter stock after September"). ⏰ TIMED FACTS still override it.
export const WINTER_ITEM_RE = /\b(hood(ie|y|ies)|sweat\s*shirts?|winter|sardi|sardiyon|zip(per)?s?|jackets?|varsity|pullovers?)\b/i
export function winterStockLine(now = new Date()) {
  const m = Number(new Intl.DateTimeFormat('en-US', { month: 'numeric', timeZone: 'Asia/Kolkata' }).format(now))
  if (m >= 9 && m <= 11) return 'Winter ka stock aana start ho gaya hai sir — jo website pe dikh raha hai wahi abhi available hai, baaki aata rahega; purchase karte rahiye 🙏'
  if (m === 12 || m <= 2) return 'Winter ka stock aa raha hai sir — jo website pe dikh raha hai wahi abhi available hai, baaki Coming Soon tab mein check kar lijiye 👉 https://www.bulkplaintshirt.com/delhi-stock.html'
...
async function runAiFlow({ whatsappNumber, mergedText, quotedText, conversationId, normalizedText, db, anthropic, settings, startTime, messageIds, imageUrl = null, imageMessages = [], textOnlyInput = false }) {
  ...
  let forcedReply = !!imageBlock || FORCE_REPLY_RE.test(mergedText || '') || isAppStoreLookupProblem(mergedText) || isRecipientReceiptQuestion(mergedText) || isNearestMetroQuestion(mergedText)
...
Answer with ONLY one word: REPLY or SILENT.` }],
    ...
    if (verdict.startsWith('SILENT') && isPrinterFulfilmentFollowup(mergedText, recentForGate)
        && !(await ketuRepliedLast(db, conversationId))) {
      verdict = 'REPLY'
...
- BILLS / INVOICES — "bill bhejdo", "invoice nahi mila" → the login link (sale91.com/login). LEDGER / STATEMENT / ALL BILLS — "saare bills chahiye", "ledger bhejo", "statement", "since starting", "purchase summary" → the self-serve ledger page https://www.bulkplaintshirt.com/ledger.html (verify by OTP on WhatsApp or e-mail, download all bills as one PDF/Excel; no password needed).
...
Reply with exactly one word: KETU or ASSISTANT.`,
  ...
  // A carried-over defer: tell the model which part of the earlier question is already answered
  // (partial defer), so it does not repeat the price and only answers / [DEFER]s what is still open.
  const carriedHere = carriedDeferByNumber.get(whatsappNumber)
  const carriedNote = carriedHere
    ? (carriedHere.messageText
        ? `⏳ CARRIED-OVER QUESTION: the buyer sent this EARLIER and it is still unanswered — "${String(carriedHere.messageText).slice(0, 300)}" — it is included at the start of the message below, followed by the new message`
        : `⏳ RE-SENT QUESTION: the buyer sent this same message EARLIER and it is still unanswered`)
      + `${carriedHere.carriedAnswered ? `. This part of it you ALREADY answered: "${String(carriedHere.carriedAnswered).slice(0, 300)}" (do not repeat it)` : ''}. Answer what you can now; put [DEFER] on its own last line only for what still needs Ketu.\n\n`
    : ''
  ...
  const STOCK_INTENT_RE = new RegExp(
    'stock|avail|milega|milegi|mil\\s*(jayega|sakta|sakti)|restock|refill|\\bupdate\\b|aa\\s*(gaya|gay[ei]|jayega|raha)|aayega|ayega|kab\\s*(aa|tak|milega|mileg)|nahi\\s*(dikh|aa|mil)|nhi\\s*(dikh|aa|mil)|dikh(a|ta)?\\s*n[ae]?hi|show\\s*nahi|not\\s*showing'  // 2026-09-08: 'polo navy 42 44 update' and 'Red bhi abhi nahi dikha rha' had no block
  ...
  // EXPORT / OUTSIDE-INDIA ASK (2026-09-05): see EXPORT_ASK_RE — a foreign destination in the
  // message gets the 🌍 line so the model answers the export rule instead of deferring the quote.
  ...
  // ❄️ WINTER STOCK (2026-09-02): seasonal line computed for today — see winterStockLine above.
  // Injected only when a winter item is mentioned, so the model never volunteers winter talk.
  if (WINTER_ITEM_RE.test(mergedText || '')) {
  ...
  const langAsk = /\b(in english|english (please|pls|plz)|english me(?:in)?\s*(bolo|batao|reply|likho)?|(please|pls|plz) english|reply in english|only english)\b/i.test(mergedText || '') ? 'english'
  ...
  if (preBrainCooldown?.cooldownUntil && new Date() < new Date(preBrainCooldown.cooldownUntil)) {
    cancelPendingDefer(whatsappNumber, 'defer_superseded_by_intervention')
    await createLog(db, conversationId, mergedText, messageIds, {
      status: 'SKIPPED',
  ...
  if (aiReply.includes('[SKIP]')) {
    ...
    await db.settings.update({ where: { id: 'default' }, data: { dailySpentUsd: { increment: costUsd } } })
    // Auto-learn only a short, plain ender — never a burst that carries a deferred question
    // ("stock refill kab hoga ok" is not an acknowledgment phrase; review 2026-09-02).
    const carriedHere = carriedDeferByNumber.get(whatsappNumber)
  ...
  try {
    ...
    const billLoginIdentity = billLoginIdentityGuard({ reply: aiReply, imageUrl })
    if (billLoginIdentity) {
      aiReply = billLoginIdentity
      console.log(`[BillLogin] ${whatsappNumber} — buyer bill login identity corrected`)
    }
    ...
    if (labelReferral) {
      aiReply = labelReferral
      console.log(`[CustomLabelReferral] ${whatsappNumber} — existing independent vendor referral restored`)
    }
    ...
    const regularFitPriceReply = regularFitRateSummaryGuard({ products: catalogProducts, buyerText: mergedText, history: conversationHistory, reply: aiReply, imageUrl, english: buyerUsesEnglish({ buyerText: mergedText, history: conversationHistory }) })
    ...
    if (pendingChoiceReply) {
      aiReply = pendingChoiceReply
      console.log(`[ProductChoiceGuard] ${whatsappNumber} — kept unanswered product choice before timing`)
    }
    ...
    if (stockAlertOffer) {
      ...
      console.log(`[StockAlertOffer] ${whatsappNumber} — restock next action applied`)
    }
    ...
    if (restockHandoff) {
      ...
      restockPointerHeld = true
      console.log(`[RestockPointerGuard] ${whatsappNumber} — repeated no-date pointer held for owner`)
    }
  ...
  // --- DELIVERY-DAYS GUARD (2026-09-09) — see deliveryDaysGuard ---
  {
    const fixed = deliveryDaysGuard({ buyerText: mergedText, reply: aiReply })
    if (fixed) {
      console.log(`[DeliveryDaysGuard] ${whatsappNumber} — invented day figure replaced: "${String(aiReply).slice(0, 80)}"`)
      aiReply = fixed
    }
  }
  ...
  {
    const guarded = arrivalClockGuard({ buyerText: mergedText, reply: aiReply })
    if (guarded) {
      aiReply = guarded
      arrivalClockBlocked = true
  ...
  // PARTIAL DEFER (2026-09-02): only a BARE [DEFER] is overridden here. "answer + [DEFER]" already
  ...
  // PARTIAL DEFER (2026-09-02, see reconcile.js): "answer + [DEFER]" means send the answer now and
  // hold the rest — the holding line follows on the defer batch and the question lands in Waiting.
  // Before this, any [DEFER] anywhere collapsed the whole reply into a holding line, so a buyer
  // writing "not received yet — hoodie price?" got either the price OR the hold, never both.
  let partialDeferHeld = false
  {
    const split = deferSplit
    if (split.isDefer) {
      scheduleDeferReply({
        whatsappNumber, deferMessage: settings.deferMessage, conversationId,
        mergedText, messageIds, logData: {
      ...
      aiReply = split.text   // fall through: the answerable part is sent like any reply
      partialDeferHeld = true
      console.log(`[PartialDefer] ${whatsappNumber} — answering the answerable part, holding the rest`)
    }
  }
  ...
  // Applies to WhatsApp and Instagram alike, so it sits above the IG-only scrubs below.
  aiReply = scrubDispatchClock(aiReply)
  ...
  if (isInstagram) {
    ...
    try {
      ...
      if (!lastInboundMs || Date.now() - lastInboundMs > 24 * 60 * 60 * 1000) {
        ...
        await db.settings.update({ where: { id: 'default' }, data: { dailySpentUsd: { increment: costUsd } } })
        // A partial defer scheduled above would try a holding line Meta will reject — drop it (logged).
        cancelPendingDefer(whatsappNumber, 'ig_window_expired')
  ...
  // (2026-09-02) This used to cancelPendingDefer() here — "AI is engaging with the buyer" — which
  // silently killed an EARLIER burst's queued holding line. Any earlier defer is now carried into
  // the burst at enqueue time, and a PARTIAL defer scheduled just above must survive this send.
  ...
  if (!sendCtx.blocked) carriedDeferByNumber.delete(whatsappNumber)   // the model read and answered the carried question
  await createLog(db, conversationId, mergedText, messageIds, {
...
// The per-request clock block at the top of every user prompt. Exported so the replay harness
// can reproduce a specific moment (case.at) — time-dependent misses (after-hours "ship today",
// late-night call answers) were untestable before 2026-09-06.
export function istTimeBlock(nowMs = Date.now()) {
  let prompt = ''
  ...
  const _nowIST = new Date(nowMs + _IST_MS)
  const _tomIST = new Date(nowMs + _IST_MS + 86400000)
  ...
  // The same-day dispatch promise is only honest while the godam is open; say so in the clock line.
  const _dow = _nowIST.getUTCDay()
