...
export function holdingLineRepeatFromRows(rows) {
  for (const r of rows || []) {
    ...
    if (r.status === 'REPLIED' && r.deferReason !== 'welcome_followup_generic' && r.deferReason !== 'claude_partial_answer') return false  // a real answer went out (a partial's own answer is not one for the held part)
