...
const SIMPLE_ASK = /^(?:(?:product|180\s*gsm|hi|hello|sir|bhai|please|pls|plz|share|send|tell|me|the|your|ka|ki|ke|k|kya|hai|hain|h|rate|rates|price|prices|list)\s*)+$/i
...
export function biowashRateSummaryGuard({ products = [], buyerText, history = [], reply, english = false }) {
  ...
  if (quoted.length !== 1 || !!quoted[0][1] !== !!named[0][1] || output.length > 250) return null
  if ((output.match(/(?:₹\s*|\brs\.?\s*)\d+(?:\.\d+)?/gi) || []).length !== 1) return null
  ...
  if (words.some(word => !/^(?:product|bulk|pcs?|pieces?|per|each|is|are|at|for|in|the|price|prices|rate|rates|hai|hain|h|sir|bhai|pe|mein|ka|ke|ki)$/i.test(word))) return null
  ...
  return `${named[0][1] ? 'True Bio' : 'Bio'} Rneck 180gsm ${price} bulk (10+ total pcs, ${english ? 'by colour/size' : 'colour/size ke hisaab se'}) ${address} 👉 https://sale91.com/catalog/p/${slug}`
}
