const SIMPLE_POLO_ASK = /^(?:(?:hi|hello|sir|please|pls|plz|share|send|tell|me|the|your|both|ka|ki|ke|kya|hai|hain|rate|rates|price|prices|list|polo|polos)\s*)+$/i
...
export function poloRateSummaryGuard({ products = [], buyerText, history = [], reply, english = false }) {
  ...
  const summaryOnly = output.replace(/https?:\/\/\S+/gi, '').replace(/(?:₹\s*|\brs\.?\s*)\d+(?:\.\d+)?/gi, '').replace(/\b10\s*\+/g, '').replace(/\b\d{3}\s*gsm\b/gi, '')
