...
export function formatStockBlock(snapshot, opts = {}) {
  ...
  lines.push('HOW TO ANSWER FROM THIS DATA: each colour in IN STOCK already shows its EXACT orderable sizes — if the asked size appears next to that colour, it IS available; colours are INDEPENDENT, so NEVER say a size is out for one colour just because another colour has it out (e.g. do not claim Maroon 46 is out because Navy/Royal Blue 46 are out). Only call a size out if it is missing from that colour\'s IN STOCK sizes or explicitly in OUT OF STOCK for that exact colour. (1) asked colour/size IS in the IN STOCK list and NOT in OUT OF STOCK → "available hai sir" + order link. (2) It IS in OUT OF STOCK → "abhi out of stock hai sir"; then obey THAT ENTRY\'S OWN restock verdict, which is already resolved for you — "↳ ... arriving ~N din" → you MAY say "~N din mein aa jayega sir"; "⛔" → say NO date at all, not even "jaldi"/"soon"/"thoda time", and briefly offer the WhatsApp stock-alert form instead 👉 https://www.bulkplaintshirt.com/delhi-stock.html?alert=1. ⚠️ NEVER carry an ETA across a colour or a size: a date shown for one colour of a product does NOT apply to another colour of that product, and a shipment\'s sizes do NOT cover sizes missing from it (2026-08-31, buyer 9764372985: the clone read "Kids Rneck — Mustard Yellow ~6 din" and promised Kids BLACK 24 in ~6 days, and read "True Bio Rneck — Black" to promise Navy 38 — neither colour had any shipment; Ketu: "there is nothing in transit for Black 24, how are you saying that?"). An invented arrival date is the single most damaging thing you can say — the buyer waits, nothing comes. If the verdict is ⛔ or you are unsure the row matches the EXACT product+colour+size asked, give no date; suggest 1-2 alternatives — ONLY colours/sizes that are genuinely orderable (use each OUT-OF-STOCK entry\'s "still available:" sizes; NEVER suggest a size/colour listed as out; if the buyer asked for a SPECIFIC size, suggest only colours that have THAT size available). (2b) The buyer asks WHEN something comes back ("kab tak aayegi", "kab aayega", "kab tak available ho jaayegi", "when will it be available / restock") → they want the TIMING, so LEAD with the ETA from that OUT-OF-STOCK entry\'s own verdict ("~N din mein aa jayega sir") — but ONLY if the verdict shows "↳" for the exact colour AND size asked; if it shows "⛔", the honest answer is that you cannot give a date; briefly offer the WhatsApp alert, NOT a borrowed one. Then add what is available meanwhile. Listing only what is in stock IGNORES the question (Ketu 2026-07-27: "acid wash kab tak available ho jaaegi" got a list of available colours; his own answer was the ETA — "Black XL in 2 to 3 days"). If NO COMING SOON row matches that product/colour, do NOT invent a date — briefly say the date is not known and offer the WhatsApp alert, not another instruction to keep checking Coming Soon. For an offered item that is out of stock, briefly offer the WhatsApp alert even when giving its exact verified ETA. Do not repeat an alert already shared for the same item; if the buyer still needs an unknown date, retain the owner handoff. Never offer restock alerts for discontinued/non-made items. (3) Product/colour NOT confidently identifiable in this data (naming doubt, variant doubt) → [DEFER] as usual. NEVER state quantities. If Ketu said something different in this thread, HIS word wins.')
  return lines.join('\n')
}
...
export const PRODUCT_NAMED_RE = /\b(bio|true\s*bio|non\s*bio|polo|oversize[d]?|over\s*size|drop\s*shoulder|acid|acidwash|hoodie|hoody|sweat\s*shirts?|kids?|bachch?o?n?|sublimation|shorts?|zip|zipper|varsity|jacket|boxy|\d{3}\s*gsm|os\s*-?\s*\d{3}|(180|200|210|220|240|260|320|430)|rneck|round\s*neck|premium|cotton\s*polo)\b/i
