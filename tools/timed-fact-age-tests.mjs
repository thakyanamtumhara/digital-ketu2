...
const unnamed = content('Kab tak out of stock hai?', '11-13 दिन में आ जाना चाहिए', '2026-09-11')
assert.equal(formatTimedFactsBlock([unnamed], now), null)
assert.equal(formatTimedFactsBlock([content('When will it be available?', 'Cotton Polo Grey in 11-13 days', '2026-09-11')], now), null)
assert.equal(formatTimedFactsBlock([content('When will it be available? [product photo]', '11-13 days', '2026-09-11')], now), null)
const scoped = formatTimedFactsBlock([unnamed, ...facts, content('Women range launch estimated time?', '30 to 45 days max', '2026-09-06')], now)
assert.doesNotMatch(scoped, /11-13|Kab tak out of stock/)
assert.match(scoped, /Oversize 240gsm Red restock/)
assert.match(scoped, /4-5 days/)
assert.match(scoped, /Women range launch[^\n]*25-40 days/)
...
const crossed = content('240gsm Red restock?', 'Take another GSM; बाकी एसिड वाश में 6-7 दिन लगेंगे।', '2026-09-11')
assert.equal(currentTimedFact({ date: '2026-09-11', question: 'Polo restock?', answer: 'Hoodie in 6 days' }, now), null)
assert.equal(formatTimedFactsBlock([crossed], now), null)
const crossedStock = formatStockBlock(snapshot, { timedFacts: [crossed], now })
assert.doesNotMatch(crossedStock, /⏰ Ketu said|6-7 days/)
assert.match(crossedStock, /Red \[out: M[^\n]*NO shipment/)
const exactSubject = content('AcidWash 240gsm Red restock?', 'Take another GSM; बाकी एसिड वाश में 6-7 दिन लगेंगे।', '2026-09-11')
assert.match(formatTimedFactsBlock([exactSubject], now), /6-7 days/)
assert.match(formatTimedFactsBlock([content('AcidWash Oversize Red restock?', '6-7 दिन लगेंगे', '2026-09-11')], now), /6-7 days/)
assert.match(formatTimedFactsBlock([crossed, ...facts], now), /4-5 days/)
assert.doesNotMatch(formatTimedFactsBlock([crossed, ...facts], now), /6-7 days/)
for (const phrase of ['आठ से दस दिन', 'आट से दस दिन', 'आठ-दस दिन', 'आठ – दस दिन']) {
  const aged = currentTimedFact(fact(`${phrase} में आएगा`), now)
  assert.equal(aged.answer, '4-6 दिन में आएगा')
  assert.equal(aged.timingEstimate, '4-6 days')
  assert.equal(currentTimedFact(fact(phrase, '2026-09-11'), now).timingEstimate, '8-10 days')
  assert.equal(currentTimedFact(fact(phrase, '2026-09-03'), now), null)
}
assert.equal(currentTimedFact(fact('कम से कम आठ से दस दिन'), now).timingEstimate, 'minimum 4-6 days')
for (const answer of ['आठ से दस दिन, फिर 12 दिन', '8 days या आठ से दस दिन', 'आठ से दस दिन और आठ-दस दिन', 'साढ़े आठ से दस दिन']) assert.equal(currentTimedFact(fact(answer), now), null)
assert.equal(currentTimedFact(fact('पाठ से दस दिन'), now).adjusted, false)
assert.equal(currentTimedFact(fact('आठ से दस दिनों का पुराना उदाहरण'), now).adjusted, false)
const hindiFacts = [content('Oversize 240gsm Red restock?', 'आट से दस दिन में आएगा')]
assert.match(formatTimedFactsBlock(hindiFacts, now), /Current timing estimate: "4-6 days"/)
assert.doesNotMatch(formatTimedFactsBlock(hindiFacts, now), /आट|आठ|8-10/)
const hindiStock = formatStockBlock(snapshot, { timedFacts: hindiFacts, now })
assert.match(hindiStock, /Red \[out: M[^\n]*4-6 days/)
assert.match(hindiStock, /White \[out: M[^\n]*NO shipment/)
assert.match(hindiStock, /Kids Rneck: Red \[out: 24[^\n]*NO shipment/)
assert.doesNotMatch(hindiStock, /आट|आठ|dated estimate/)
